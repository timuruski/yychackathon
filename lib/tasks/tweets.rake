namespace :tweets do
  ...
  desc 'Create sample tweets'
  task :seed => [:environment] do
    10.times do
      InterestingTweet.create do |tweet|
        raw = { }

        tweet.tweet_id = 'abc123'
        tweet.username = 'timuruski'
        tweet.image_url = 'http://example.org/kittens.jpg'
        tweet.created_at = Time.current
        tweet.text = 'Can haz cheezeburger?'
        tweet.latitude = 0.00
        tweet.longitude = 0.00
        tweet.raw = raw
      end
    end
  end
